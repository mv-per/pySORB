...
// PurePTA::PurePTA(std::string potential, std::string equation_of_state,
...
//  */
// double PurePTA::GetLoading(double P, double T, std::vector<double>
// potential_params, Fluid Fluid)
// {
//     this->Fluid = Fluid;
//     call_potential PotentialInvoker =
//     GetAdsorptionPotentialInvoker(potential_params, Fluid, this->adsorbent);
//     call_mono_eos eos_caller = GetEquationOfStateInvoker(Fluid);
//     double Loading = this->Loader(P, T, potential_params, eos_caller,
//     PotentialInvoker); return Loading;
// }
...
//  * @return Calculated adsorbed loading
//  */
// double PurePTA::GetPressure(double n, double T, std::vector<double>
// potential_params, Fluid Fluid, double P_estimate_ = 1e6)
// {
//     this->Fluid = Fluid;
//     call_potential PotentialInvoker =
//     GetAdsorptionPotentialInvoker(potential_params, Fluid, this->adsorbent);
...
//     {
//         double diff = (this->Loader(x, T, potential_params, eos_caller,
//         PotentialInvoker) - n); return diff * 1000;
//     };
...
//  */
// std::vector<double> PurePTA::GetLoadings(std::vector<double> P, double T,
// std::vector<double> potential_params, Fluid Fluid)
// {
//     std::vector<double>
//         loadings(P.size());
...
//     {
//         loadings[i] = this->Loader(P[i], T, potential_params, eos_caller,
//         PotentialInvoker);
//     }
...
//                                   std::vector<double> loading_exp,
//                                   std::vector<double> P, double T,
//                                   std::vector<double> potential_params, Fluid
//                                   Fluid)
// {
...
//     std::vector<double> calc_loading = this->GetLoadings(P, T,
//     potential_params, Fluid); for (std::size_t i = 0; i < P.size(); i++)
//     {
...
std::function<mono_eos(double, double)>
GetPureEquationOfStateInvoker(std::string equation_of_state, Fluid fluid_) {
...
GetPureAdsorptionPotentialInvoker(std::string potential, Fluid Fluid,
                                  Adsorbent adsorbent) {
  if (potential == DRA_POTENTIAL) {
    return [=](double z, std::vector<double> potential_params) {
      return DRA(z, potential_params[0], potential_params[1],
                 potential_params[2]);
    };
  } else if (potential == STEELE_POTENTIAL) {
    return [=](double z, std::vector<double> potential_params) {
      return STEELE(z, potential_params[0], Fluid.LennardJonnesDiameter,
                    adsorbent);
    };
  } else if (potential == LEE_POTENTIAL) {
    return [=](double z, std::vector<double> potential_params) {
      return LEE(z, potential_params[0], Fluid.LennardJonnesDiameter,
                 adsorbent);
    };
  } else {
...
double GetDRAPureLoading(double BulkPressure, double Temperature,
                         std::vector<double> Parameters,
                         call_mono_eos EquationOfStateInvoker,
                         call_potential PotentialInvoker, Fluid Fluid,
                         std::size_t NumberOfLayers, std::string IsothermType,
  ...
  std::vector<double> z = linespace(Parameters[1], delta, NumberOfLayers);
  ...
  for (std::size_t i = 0; i < z.size(); i++) {
    ...
    integral[i] =
        GetCalculatedAdsorbedDensity(adsorbed.dens, bulk.dens, IsothermType) *
        1e-3;
  }
  return integrate(integral, delta);
}

double GetLJPureLoading(double BulkPressure, double Temperature,
                        std::vector<double> Parameters,
                        call_mono_eos EquationOfStateInvoker,
                        call_potential PotentialInvoker, Fluid Fluid,
                        std::size_t NumberOfLayers, std::string IsothermType,
  ...
  if (!Fluid.LennardJonnesDiameter) {
    throw std::invalid_argument(
        "Missing the Lennard-Jonnes diameter for the Fluid " + Fluid.Name);
  }
  double minimal_space = 0.7 * Fluid.LennardJonnesDiameter;

  std::vector<double> z =
      linespace(Parameters[1] / 2.0, minimal_space, NumberOfLayers);
  ...
  for (std::size_t i = 0; i < z.size(); i++) {
    eps = PotentialInvoker(z[i], Parameters) +
          PotentialInvoker(Parameters[1] - z[i], Parameters);
    f_eps = GetAdsorptionPotentialEnergy(eps, Temperature, Potential);
    Pz = solver.findOptimizedPressure(Pz, f_eps);
  ...
  return -2.0 * Parameters[2] * integrate(integral, delta) * 1e-7;
}
